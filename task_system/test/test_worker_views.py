...
WORKER_URL = reverse("task_system:worker-list")
...
class PrivateWorkerTest(TestCase):
    ...
    def test_detail_worker(self):
        res = self.client.get(reverse(
            "task_system:worker-detail", args=[1]
        ))
        self.assertEqual(res.status_code, 200)

    def test_create_workers(self):
        response = self.client.post(
            reverse("task_system:worker-create"),
            {
        ...
        self.assertRedirects(response, reverse("task_system:index"))
        self.assertTrue(
    ...
    def test_update_worker(self):
        res = self.client.post(
            reverse_lazy("task_system:worker-update", args=[1]),
            {
